...
import { richesseLevel } from './richesse';
...
const OUT = process.env.ENRICH_OUT ?? `${SCRATCH}/enrich-test-100.outcomes.json`;

interface Enriched {
  id: number; name: string; naf: string; lat: number; lng: number; etat: string;
  place_id?: string; g_name?: string; g_address?: string; rating?: number; reviews?: number;
  phone?: string; website?: string; types?: string[]; business_status?: string;
  g_lat?: number; g_lng?: number; photo_ref?: string; photo_refs?: string[];
  opening_hours?: unknown; pauvre_au_find?: boolean;
}
...
(existsSync(DATA) ? describe : describe.skip)('rendement enrichissement — chaîne complète', () => {
  it('trie les 100 enrichis', () => {
    ...
    const out = rows.map((r) => {
      ...
      let sortie: 'PUBLIABLE' | 'QUARANTAINE' | 'REJET_REVELE' | 'INTROUVABLE' | 'REPORTE_C';
      ...
      if (r.etat !== 'TROUVE') { sortie = 'INTROUVABLE'; raisons.push('aucune fiche Google exploitable — reste vérifiée nue'); }
        ...
        // Décision A/B (12/07) : la richesse pilote la DÉPENSE/écriture, jamais l'existence.
        const dist = r.g_lat != null ? distM(r.lat, r.lng, r.g_lat, r.g_lng!) : Infinity;
        const niveau = r.pauvre_au_find ? 'C' : richesseLevel({
          etat: 'TROUVE', photo_ref: r.photo_ref, opening_hours: r.opening_hours,
          phone: r.phone, website: r.website, adresse_ok: dist <= 300,
          types: r.types, rating: r.rating, reviews: r.reviews, description: null,
        });
        const concordance = overlap(tokens(r.name), tokens(r.g_name ?? '')) >= 0.5 || dist <= 120;
        if (!concordance) {
          // Classe « collision de voisinage » : jamais d'écriture sans concordance POSITIVE.
          sortie = 'QUARANTAINE';
          raisons.push(`concordance non démontrée : « ${r.g_name} » à ${Math.round(dist)} m, noms disjoints`);
        } else if (niveau === 'A' || niveau === 'B') {
          sortie = 'PUBLIABLE';
          raisons.push(`${decision.source} → ${decision.category} · niveau ${niveau} · ★${r.rating ?? '—'} (${r.reviews ?? 0}) · photo ${r.photo_ref ? '✓' : '—'}`);
        } else {
          sortie = 'REPORTE_C';
          raisons.push(`fiche Google pauvre (niveau ${niveau}) — reportée, aucune écriture, reste voie vérifiée`);
        }
        return { ...r, sortie, raisons, categorie: decision.category, gType, niveau };
      }
      return { ...r, sortie, raisons, categorie: decision.category, gType };
    });
    const n = (s: string) => out.filter((x) => x.sortie === s).length;
    writeFileSync(OUT, JSON.stringify(out, null, 1));
    console.log(`PUBLIABLE=${n('PUBLIABLE')} REPORTE_C=${n('REPORTE_C')} QUARANTAINE=${n('QUARANTAINE')} REJET_REVELE=${n('REJET_REVELE')} INTROUVABLE=${n('INTROUVABLE')}`);
